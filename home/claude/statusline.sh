#!/bin/bash
# Inspired by https://github.com/fatih/dotfiles/blob/main/claude/statusline-git.sh
...
# Build context progress bar (15 chars wide + threshold marker)
bar_width=15
threshold_percent=95
threshold_pos=$((threshold_percent * bar_width / 100))  # 14

filled=$((context_percent * bar_width / 100))
bar=""

for ((i=0; i<bar_width; i++)); do
    # Insert red threshold marker at 95% position
    if [ $i -eq $threshold_pos ]; then
        bar+="${RED}│${GRAY}"
    fi

    # Add normal bar character
    if [ $i -lt $filled ]; then
        bar+="█"
    else
        bar+="░"
